/* Ambient ocean layers — bubbles, light rays, plankton, drifting fish, fronds */
...
// ─── Continuous bubble column rising the full page
function Bubbles({ count = 28 }) {
  const bubbles = useMemo(() =>
    Array.from({ length: count }).map((_, i) => {
      const size = 4 + seeded(i, 2) * 18;
      return {
        left: seeded(i, 3) * 100,
        size,
        delay: -seeded(i, 4) * 18,
        ...
        // bubbles deeper down should be slightly dimmer
...
// ─── Drifting plankton specks (small luminous dots)
function Plankton({ count = 60 }) {
  const items = useMemo(() =>
    Array.from({ length: count }).map((_, i) => ({
...
// ─── Caustic light rays anchored to viewport, parallax-shift on scroll
function LightRays() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        // fade rays out as we descend
        const fade = Math.max(0, 1 - y / 1800);
...
// ─── Fish silhouette SVG (abstract tropical, no literal beach motif)
function FishSVG({ color = 'rgba(255,255,255,0.5)' }) {
  return (
    <svg width="64" height="28" viewBox="0 0 64 28" fill="none">
      <path
        d="M2 14 C 10 4, 30 4, 44 14 C 30 24, 10 24, 2 14 Z"
        fill={color}
      />
...
// ─── Schools of drifting fish at varied depths
function FishLayer() {
  const fish = [
...
// ─── Wavy SVG divider between sections (subtle, no literal beach)
function WaveDivider({ from = 'transparent', to = 'rgba(255,255,255,0.04)', flip = false, height = 90 }) {
  return (
    <svg
    ...
    >
      <defs>
        <linearGradient id={`wg-${flip ? 'f' : 'u'}`} x1="0" y1="0" x2="0" y2="1">
...
// ─── Coral / kelp frond SVG — drifting at the page edges
function FrondSVG({ tone = 'reef' }) {
  // Two tones of frond — coral (warm) and kelp (cool)
  const fills = {
    reef:  ['#0c5d68', '#16a89e', '#9af0d0'],
    coral: ['#7a3a2c', '#ff8a6b', '#ffd28a'],
    kelp:  ['#053640', '#0a7780', '#5fdcd0'],
  }[tone] || ['#0c5d68', '#16a89e', '#9af0d0'];

  return (
    <svg width="120" height="260" viewBox="0 0 120 260" fill="none">
      <path d="M60 260 C 70 200, 50 160, 65 110 C 75 70, 50 40, 60 0"
            stroke={fills[1]} strokeWidth="6" strokeLinecap="round" fill="none" opacity="0.55" />
      {/* small leaves */}
      {[0, 1, 2, 3, 4, 5].map(i => {
        const y = 240 - i * 38;
      ...
      {/* base */}
      <ellipse cx="60" cy="258" rx="28" ry="6" fill={fills[0]} opacity="0.6" />
    </svg>
  );
}

// ─── Sun-dappled surface ripple (for hero top)
function SurfaceRipple() {
  return (
    <div className="absolute top-0 inset-x-0 pointer-events-none" style={{ height: 140 }}>
      ...
      >
        <path
          d="M0,30 C240,60 480,0 720,25 C960,50 1200,5 1440,30 L1440,0 L0,0 Z"
          fill="rgba(255, 250, 210, 0.18)"
        />
        <path
          d="M0,70 C240,40 480,90 720,65 C960,40 1200,90 1440,60 L1440,0 L0,0 Z"
...
Object.assign(window, {
  Bubbles, Plankton, LightRays, FishLayer, FishSVG,
  WaveDivider, FrondSVG, SurfaceRipple, seeded,
});
