...
function Projects() {
  return (
    <section id="projects" data-screen-label="03 Projects" className="relative py-28">
      {/* Floating fish school in background */}
      <FishLayer />

      {/* Coral fronds at corners */}
      <div className="absolute pointer-events-none hidden md:block"
           style={{ top: '15%', left: '-30px', opacity: 0.45 }}>
              ...
              <div className="glass-card rounded-3xl p-6 flex flex-col lg:flex-row gap-6 hover:-translate-y-1 transition-all duration-500 group"
                ...
                <div className="lg:w-80 shrink-0 relative aspect-video rounded-xl overflow-hidden border border-white/10">
                  ...
                  <span className="absolute top-3 left-3 text-xs font-bold px-2.5 py-1 rounded-full"
                        style={{ background: 'var(--seafoam)', color: '#093d33' }}>
                    Live
                  </span>
                  {/* underwater overlay */}
        ...
        <Reveal variant="scale">
          <div className="hidden md:block rounded-3xl overflow-hidden glass-card"
            ...
            <div className="grid grid-cols-4 border-b border-white/10"
                 style={{ background: 'rgba(8, 50, 63, 0.4)' }}>
              <div className="p-5" />
            ...
            {COMPARISON.map((r, i) => (
              <div key={r.feature}
                ...
                <div className="p-4 flex items-center justify-center border-l border-white/10"
                     style={{ background: 'rgba(154, 240, 208, 0.06)' }}>
                  <Cell v={r.codeMack} />
                </div>
              ...
              <div className="glass-card rounded-2xl p-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="flex flex-col items-center gap-1.5 rounded-xl px-2 py-3"
                       style={{ background: 'rgba(154, 240, 208, 0.1)' }}>
                    <Cell v={r.codeMack} />
...
// ─── Contact
const PROJECT_TYPES = ['Landing Page', 'Business Site', 'Custom Web App', 'Website Redesign', 'Other'];

function Contact() {
  const [form, setForm] = _useState({ name: '', email: '', projectType: '', message: '' });
                ...
                <form onSubmit={submit} className="glass-card rounded-3xl p-7 sm:p-8 space-y-5"
                  ...
                  <Field label="Project Type">
                    <select required value={form.projectType}
                            ...
                            className="w-full px-4 py-3 rounded-xl bg-white/8 border border-white/15 text-white focus:outline-none focus:border-white/40 focus:bg-white/12 transition-all text-sm appearance-none"
                      ...
                      <option value="" disabled style={{ background: '#08323f' }}>Select a project type</option>
                      {PROJECT_TYPES.map(t => <option key={t} value={t} style={{ background: '#08323f' }}>{t}</option>)}
                    </select>
                  </Field>
